...
public abstract class Minesweeper
{
    ...
    public void ClickIn(int x, int y)
    {
        if (InRange(x, y))
        {
            if (getState() != MinesweeperGameState.GameOver && getState() != MinesweeperGameState.GameWin)
            {
                if(getState() == MinesweeperGameState.WaitForStart)
                {
                    state = MinesweeperGameState.InGame;
                    stopwatch.start();
                }
                if (mask[x][y] == UNKNOWN || mask[x][y] == QUESTIONMARK)
                {
                    if (filed[x][y] == MINE)
                    {
                        mask[x][y] = FAIL;
                        FindWrongMineMarks();
                        state = MinesweeperGameState.GameOver;
                        stopwatch.stop();
                    }
                ...
                if (IsWin())
                {
                    state = MinesweeperGameState.GameWin;
                    stopwatch.stop();
                }
            }
        }
    ...
    {
        return mask[x][y] >= ZERO;
    }

    public boolean IsUnknownCell(int x, int y)
    {
        return mask[x][y] < MINE && mask[x][y] >= WRONGMINEMARK;
    }
    public boolean IsFailCell(int x, int y)
    {
        return mask[x][y] == FAIL;
    }
     ...
     * @return the state
     */
    public MinesweeperGameState getState()
    {
        return state;
    }
}
